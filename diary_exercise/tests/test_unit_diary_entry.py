...
"""
Test reading chunk restarts from the beginning again after the contents have been read
"""
def test_reading_chunk_resets():
    ...
    chunk1 = entry.reading_chunk(wpm, minutes)
    chunk2 = entry.reading_chunk(wpm, minutes)
    entry._current_index = 0
    chunk3 = entry.reading_chunk(wpm, minutes)

    assert chunk1 == "'It does not matter how slowly you go"
    assert chunk2 == "as long as you do not stop.' Confucius"
    assert chunk3 == "'It does not matter how slowly you go"